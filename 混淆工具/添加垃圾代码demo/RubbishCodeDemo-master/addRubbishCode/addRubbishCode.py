...
def GetMFileEndCount(file_path,old_str):
    ...
    Ropen=open(file_path,'r',encoding='utf-8')#读取文件
    flagCount = 0
    for line in Ropen:
...
def HFileAddCode(file_path,old_str,endTotalCount):
    ...
    file_data = ""
    Ropen=open(file_path,'r',encoding='utf-8')
    flagCount = 0
    for line in Ropen:
        ...
        if old_str in line:
            ...
            file_data += line
    Ropen.close()
    Wopen=open(file_path,'w',encoding='utf-8')
...
def addRubbish(file_dir):
    ...
    for dir in file_dirs:
        file_dir = file_dir + '/' + dir
        traverse(file_dir)
        
#要修改的文件所在的文件夹路径
file_floadPath = '/Users/yangkejun/Downloads/RubbishCodeDemo-master/RubbishCodeDemo'#要修还得文件所在的文件夹路径

def main():
    addRubbish(file_floadPath)
